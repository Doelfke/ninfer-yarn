...
void scale_positions_yarn_launch(const Tensor& source, std::uint32_t original_context,
    ...
    constexpr std::int64_t block = 256;
    const std::int64_t count =
        static_cast<std::int64_t>(source.ne[0]) * source.ne[1];
    const int grid = static_cast<int>(div_up(count, block));
    scale_positions_yarn_kernel<<<grid, block, 0, stream>>>(
        static_cast<std::int32_t*>(destination.data), static_cast<std::int32_t>(count),
